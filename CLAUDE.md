...
- **Logging**: Always use the `*logging.Logger` instance — never `fmt.Printf`, `log.Printf`, or `log.Println`. In stdio mode, any write to stdout corrupts the JSON-RPC stream. The logger is created early in `createApp` and passed to all components (scheduler, executors, server). One exception: config-load warnings in `config.FromEnv` (deprecated vars, unparseable values) run before the logger exists and use `log.Printf`, which goes to stderr — safe for stdio, but it bypasses `--log-file`.
- **HTTP auth**: Optional bearer token (`--auth-token` / `MCP_CRON_SERVER_AUTH_TOKEN`, normalized by `ServerConfig.SetAuthToken`) guards the HTTP transport via constant-time middleware (`internal/server/auth.go`). Never log the token. The fail-closed rule — no non-loopback HTTP bind without a token unless `--allow-unauthenticated` — is defined once in `ServerConfig.CheckAuthPolicy()` and enforced by both `Validate()` and `MCPServer.Start()`. `IsLoopbackAddress` deliberately does not resolve hostnames. Client side, mcp.json `headers` are injected by `headerRoundTripper` (`internal/agent/mcp_tools_loader.go`), which pins itself to the configured origin so credentials never follow a cross-origin redirect. User-facing detail (precedence, accepted values, loopback forms) lives in README → Authentication.
- **Config precedence**: defaults < env < explicitly passed flags. `parseConfig` in `cmd/mcp-cron/main.go` binds each flag straight to its `config.Config` field with the env-resolved value as the flag default (`flag.XVar(&cfg.Field, name, cfg.Field, help)`), so there is no flag-to-config copy step and no zero-value sentinels; add new settings the same way. `--auth-token` alone is registered via `flag.Func` (no printable default) so the secret never appears in `-h`; an explicit blank clears it like any other flag.
