...
- **HTTP auth**: Optional bearer token (`--auth-token` / `MCP_CRON_SERVER_AUTH_TOKEN`) guards the HTTP transport via constant-time middleware (`internal/server/auth.go`). Fail-closed: both `Validate()` and `MCPServer.Start()` refuse a non-loopback bind (`config.IsLoopbackAddress`) in HTTP mode without a token unless `--allow-unauthenticated` / `MCP_CRON_SERVER_ALLOW_UNAUTHENTICATED=true` is set — `Start()` is the layer that opens the socket, so the guard holds for any caller, not just the CLI. `Start()` binds synchronously (`net.Listen` then `Serve`) so bind failures surface as errors; `ListenAddr()` exposes the bound address (port 0 in tests). Never log the token. Config precedence is defaults < env < explicitly-set flags; boolean flags only enable (documented in README). `IsLoopbackAddress` deliberately does not resolve hostnames — only the literal `localhost` and loopback IP literals count. MCP server specs in mcp.json support a `headers` map (client-side, injected via `headerRoundTripper` in `internal/agent/mcp_tools_loader.go`) so AI tasks can reach token-protected servers. Because the round tripper sits below `http.Client`'s redirect logic (which would otherwise strip `Authorization` on cross-host hops), `newHeaderInjectingClient` installs a `CheckRedirect` that refuses cross-origin redirects (same scheme/host/port or `http`→`https` upgrade only) so configured headers never reach an unconfigured origin.
