...
- **Graceful shutdown**: Scheduler tracks in-flight task goroutines with a `sync.WaitGroup`; `Stop()` blocks until all running tasks complete and persist results. Shutdown timeout is `DefaultTimeout + 1 minute`; when no default timeout is set (0), shutdown waits indefinitely for in-flight tasks (the user can send SIGKILL if impatient). Result store is closed last in `app.Stop()`, after scheduler and server.
